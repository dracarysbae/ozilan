...
function Ticker() {
  ...
  const row = [...items, ...items];
  return (
    <div className="marquee-host overflow-hidden border-b-2 border-signal bg-ink py-2">
      <div className="marquee-track flex w-max gap-11 whitespace-nowrap">
        {row.map((t, i) => (
          <span key={i} className="font-mono text-[0.78rem] uppercase tracking-[0.05em] text-paper/70">
            <span className="mr-11 text-gold">◆</span>{t}
          ...
          <div className="mx-auto flex max-w-[1400px] items-stretch gap-6 overflow-x-auto px-4 lg:px-6">
            {CATEGORIES.map((c) => (
              <div key={c.slug} className="group relative shrink-0">
                ...
                <div className="invisible absolute left-0 top-full z-50 w-60 border border-ink bg-paper opacity-0 shadow-pop transition group-hover:visible group-hover:opacity-100">
                  {c.subs.map((s) => (
                    <Link key={s.slug} href={`/arama/?k=${c.slug}&a=${s.slug}`}
                      className="block border-b border-line px-3 py-2 text-[0.82rem] last:border-0 hover:bg-paper-2">
                      {s.label}
                    </Link>
                  ))}
                </div>
