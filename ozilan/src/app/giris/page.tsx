...
import {backend,emailSignInEnabled,googleSignInEnabled} from "@/lib/backend";
...
function Auth(){
  ...
  return <div className="member-layout"><section className="member-story"><p className="editorial-kicker">OZBİRARADA HESABI</p><h1>Keşiften<br/><em>bağlantıya.</em></h1><p>İlanını paylaş. Aradığını bul. Doğrudan sahibine ulaş.</p><ol><li><b>01</b><span>Fotoğraflarınla gerçek bir ilan oluştur.</span></li><li><b>02</b><span>Favorilerin ve kayıtlı aramaların hesabında kalsın.</span></li><li><b>03</b><span>Görüşmelerini yalnızca sen ve karşı taraf okuyabilsin.</span></li></ol><span className="member-orbit" aria-hidden="true"/></section>
    ...
    {live&&!me&&!emailSignInEnabled&&<h2>Bir hesap.<br/>Bütün keşiflerin.</h2>}
    {live&&googleSignInEnabled&&<GoogleAccess next={next}/>}
    {!live?<><h2>Üyelik bağlantısı hazırlanıyor.</h2><p>Gerçek hesaplar açılmadan önce ortak veri altyapısı tamamlanıyor. Şu anda şifre veya kişisel bilgi toplamıyoruz.</p><Link href="/arama/" className="btn-primary">Örnek kataloğu incele</Link></>:me&&(!emailSignInEnabled||mode!=="yenile")&&!recovery?<><p className="editorial-kicker">HOŞ GELDİN</p><h2>Merhaba {me.name.split(" ")[0]}.</h2><Link className="btn-primary" href={next}>Devam et →</Link><Link className="btn-ghost" href="/ilan-ver/">İlan oluştur</Link></>:!emailSignInEnabled?<><p className="member-privacy">Google hesabınla giriş yaparak ilanlarını, favorilerini ve görüşmelerini cihazların arasında kullanabilirsin. <Link href="/gizlilik/">Veri kullanımı</Link></p></>:<>
