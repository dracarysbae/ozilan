...
function MarketplaceContent(){
  ...
  return <div className={`market-hub market-${area.id}`}>
    ...
    <section key={area.id} className="market-hero editorial-market-hero"><div className="editorial-market-copy"><p className="market-eyebrow">{area.eyebrow}</p><h1>{area.title.split("\n").map((line,i)=>i===0?<span key={line}>{line}<br/></span>:<em key={line}>{line}</em>)}</h1><p>{area.description}</p><div className="market-hero-actions"><a href="#market-results">{goods?"Seçkiyi keşfet":"Uzmanları keşfet"} <EditorialArrow/></a>{!goods&&<button onClick={()=>startRequest(null)}>{area.id==="freelance"?"Proje oluştur":"İhtiyacını anlat"} ↗</button>}</div><span className="market-demo-label">Keşif sürümü · Örnek {goods?"ürünler":"uzman profilleri"}</span><div className="market-editorial-foot"><span>OZBİRARADA SEÇKİSİ</span><span>{area.id==="alisveris"?"01":area.id==="cicek-hediye"?"02":area.id==="hizmet"?"03":"04"} / 04</span></div></div><div className="market-editorial-visual"><span className="market-orbit" aria-hidden="true"/><EditorialMedia scene={area.id==="cicek-hediye"?"flowers":goods?"objects":"studio"} eager><span className="editorial-photo-index">{area.id==="freelance"?"FİKİRDEN ÜRETİME":area.id==="hizmet"?"İŞİN EHLİYLE":area.id==="cicek-hediye"?"BİRİNİ DÜŞÜNDÜĞÜNDE":"GÜNLÜK HAYATIN İYİ DETAYLARI"}</span></EditorialMedia></div></section>
